...
if(isset($_SESSION['usercode']) &&isset($_SESSION['dbcode']) &&isset($_SESSION['usertoken']) && isset($_REQUEST['post_data'])){
			...
			if(json_last_error() === 0){
				...
				if(isset($response['statuscode']) && $response['statuscode'] == 0) {
					...
					$items .= "<item>
									...
									<price>
										<accountcode>".$v['accountcode']."</accountcode>
										<amazonminprice>".$v['minprice']."</amazonminprice>
										<amazonmaxprice>".$v['maxprice']."</amazonmaxprice>
										<amazonfixedprice>".$v['fixedprice']."</amazonfixedprice>
										<RRP></RRP>
	...
	if(!empty($items)){
		$request = "<updatepricequantityrequest>
					...
					<source>Amazon Manual Edit</source>
					<items>
						".$items."
					</items>
