...
if(isset($_SESSION['usercode']) &&isset($_SESSION['dbcode']) &&isset($_SESSION['usertoken'])){
	...
	if(!empty($cols_data)){
		...
		$inventorydashboard_URL = COMPETE_API_URL."SKUCloudInventoryAPI/api/SKUCloudInventory/GetSKUCloudInventory";
		$request = "<getskucloudinventoryrequest>
		...
		if(!empty($sku_details)){
			...
			if(!empty($search_query)){
				...
				foreach($sku_details as $k=>$v){
					if(is_array($v) && count($v)>0){
						foreach($v as $sk=>$sv){
							...
							$inner_array = array();
							$inner_array[] = "<input type='checkbox' style='display:block;margin:5px auto;' name='myCheckboxes[]' value='".$sv['sku']."_7' data-sku='".$sv['sku']."' data-accountcode='".$sv['fkaccountcode']."' data-marketplace='21' name='sku_checkbox[]' value='sku_".$sv['sku']."' >";
				...
				$i=0;
				foreach($sku_details['skucloudinventories'] as $sk=>$sv){
					if(isset($sv['flagid']) && $sv['flagid'] != "" && $sv['flagid'] != ""){
						$flag_sku[$sv['sku']] = $sv['flagid'];
					}
					$inner_array = array();
