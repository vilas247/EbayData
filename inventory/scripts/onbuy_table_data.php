...
if(isset($_SESSION['usercode']) &&isset($_SESSION['dbcode']) &&isset($_SESSION['usertoken'])){
	...
	if(!empty($cols_data)){
		...
		$inventorydashboard_URL = COMPETE_API_URL."ONBuyInventoryAPI/api/ONBuyInventory/GetONBuyInventory";
		$request = "<getonbuyinventoryrequest>
		...
		if(!empty($sku_details)){
			...
			if(!empty($search_query)){
				...
				foreach($sku_details as $k=>$v){
					if(is_array($v) && count($v)>0){
						foreach($v as $sk=>$sv){
							...
							$inner_array = array();
							$inner_array[] = "<input type='checkbox' style='display:block;margin:5px auto;' name='myCheckboxes[]' value='".$sv['sku']."_20' data-sku='".$sv['sku']."' data-accountcode='".$sv['accountcode']."' data-marketplace='20' name='sku_checkbox[]' value='sku_".$sv['sku']."' >";
				...
				$i=0;
				foreach($sku_details['onbuyinventories'] as $sk=>$sv){
					if(isset($sv['flagid']) && $sv['flagid'] != "" && $sv['flagid'] != ""){
						$flag_sku[$sv['sku']] = $sv['flagid'];
					}
					$inner_array = array();
