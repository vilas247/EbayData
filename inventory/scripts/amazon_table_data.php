				...
				foreach($sku_details as $k=>$v){
					if(is_array($v) && count($v)>0){
						foreach($v as $sk=>$sv){
							...
							$inner_array = array();
							$inner_array[] = "<input type='checkbox' style='display:block;margin:5px auto;' name='myCheckboxes[]' value='".$sv['sku']."_2' data-sku='".$sv['sku']."' data-accountcode='".$sv['fkaccountcode']."' data-marketplace='2' name='sku_checkbox[]' value='sku_".$sv['sku']."' >";
							if(isset($sv['childitemcount']) && intval($sv['childitemcount']) > 0){
								$inner_array[] = "<span class='fa fa-plus ng-scope expand_tr' data-sku='".$sv['sku']."' data-accountcode='".$sv['fkaccountcode']."' data-marketplace='2' ></span>";
								$i++;
							}else{
								$inner_array[] = '&nbsp;';
							}
							...
							foreach($cols_data as $k=>$v){ 
								if(isset($sv[$v['val']])){
									if($v['val'] == 'active'){
										...
										$inner_array[] = '<label style="display:flex"><input '.$disabled.' type="text" id="'.$sv['sku'].'_2" value="'.$sv['totalquantity'].'" class="form-control cmnTotQtyClass" data-marketplace="2" data-sku="'.$sv['sku'].'" ><em data-sku="'.$sv['sku'].'" class="fa fa-lg fa-location-arrow" style="color:#087cc3;"></em></label>';
									}
									elseif($v['val'] == 'minprice' || $v['val']== 'maxprice'|| $v['val']== 'fixedprice'){
										$inner_array[] = '<label style="display:flex"><strong>'.$currency.'</strong><input type="text" id="'.$v['val'].'_'.$sv['sku'].'_2" value="'.$sv[$v['val']].'" class="form-control cmnTotQtyClass" data-marketplace="2" data-sku="'.$sv['sku'].'" ></label>';
									}
									elseif($v['val'] == 'fkaccountcode'){
										if(isset($accountNames[$sv[$v['val']]])){
											$inner_array[] = $accountNames[$sv[$v['val']]];
										}else{
				...
				foreach($sku_details['inventorys'] as $sk=>$sv){
					...
					foreach($cols_data as $k=>$v){ 
						if(isset($sv[$v['val']])){
							...
							elseif($v['val'] == 'minprice' || $v['val']== 'maxprice'|| $v['val']== 'fixedprice'){
								$inner_array[] = '<label><label class="inventory-price-alignment"><span> <strong>'.$currency.'</strong> </span></label><label class="inventory-price-textbox-alignment"><input type="text" id="'.$v['val'].'_'.$sv['sku'].'_2" value="'.$sv[$v['val']].'" data-marketplace="2" data-sku="'.$sv['sku'].'" class="form-control amzMinPriceClass"></label></label>';
							}
							elseif($v['val'] == 'fkaccountcode'){
								if(isset($accountNames[$sv[$v['val']]])){
									$inner_array[] = $accountNames[$sv[$v['val']]];
