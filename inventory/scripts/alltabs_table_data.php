		...
		if(!empty($sku_details)){
			...
			if(!empty($search_query)){
				...
				foreach($sku_details as $k=>$v){
					if(is_array($v) && count($v)>0){
						foreach($v as $sk=>$sv){
							...
							$inner_array = array();
							$inner_array[] = "<input type='checkbox' style='display:block;margin:5px auto;' name='myCheckboxes[]' value='".$sv['sku']."_".$sv['fkmarketplacecode']."' data-sku='".$sv['sku']."' data-accountcode='".$sv['fkaccountcode']."' data-marketplace='".$sv['fkmarketplacecode']."' name='sku_checkbox[]' >";
							if(isset($sv['childitemcount']) && intval($sv['childitemcount']) > 0){
								$inner_array[] = "<span class='fa fa-plus ng-scope expand_tr' data-sku='".$sv['sku']."' data-accountcode='".$sv['fkaccountcode']."' data-marketplace='".$sv['fkmarketplacecode']."' ></span>";
								$i++;
							}else{
								$inner_array[] = '&nbsp;';
							}
							...
							foreach($cols_data as $k=>$v){ 
								if(isset($sv[$v['val']])){
									if($v['val'] == 'active'){
										...
										$inner_array[] = '<label style="display:flex"><input '.$disabled.' type="text" id="'.$sv['sku'].'_'.$sv['fkmarketplacecode'].'" value="'.$sv['totalquantity'].'" class="form-control cmnTotQtyClass" data-marketplace="'.$sv['fkmarketplacecode'].'" data-sku="'.$sv['sku'].'" ><em data-sku="'.$sv['sku'].'" class="fa fa-lg fa-location-arrow" style="color:#087cc3;"></em></label>';
									}
									...
									elseif($v['val'] == 'fkmarketplacecode'){
										if(isset($marketplaceNames[$sv[$v['val']]])){
											$inner_array[] = $marketplaceNames[$sv[$v['val']]];
										}else{
											$inner_array[] = $sv[$v['val']];
										}
									}else{
				...
				foreach($sku_details['inventorys'] as $sk=>$sv){
					...
					$inner_array = array();
					$inner_array[] = "<input type='checkbox' style='display:block;margin:5px auto;' name='myCheckboxes[]' value='".$sv['sku']."_".$sv['fkmarketplacecode']."' data-sku='".$sv['sku']."' data-accountcode='".$sv['fkaccountcode']."' data-marketplace='".$sv['fkmarketplacecode']."' name='sku_checkbox[]' value='sku_".$sv['sku']."' >";
					if(isset($sv['childitemcount']) && intval($sv['childitemcount']) > 0){
						$inner_array[] = "<span class='fa fa-plus ng-scope expand_tr' data-sku='".$sv['sku']."' data-accountcode='".$sv['fkaccountcode']."' data-marketplace='".$sv['fkmarketplacecode']."' ></span>";
						$i++;
					}else{
