		...
		if(!empty($sku_details)){
			...
			if(!empty($search_query)){
				...
				foreach($sku_details as $k=>$v){
					if(is_array($v) && count($v)>0){
						foreach($v as $sk=>$sv){
							...
							$inner_array = array();
							$inner_array[] = "<input type='checkbox' style='display:block;margin:5px auto;' name='myCheckboxes[]' value='".$sv['sku']."_3' data-sku='".$sv['sku']."' data-accountcode='".$sv['fkaccountcode']."' data-marketplace='3' name='sku_checkbox[]' >";
							if(isset($sv['childitemcount']) && intval($sv['childitemcount']) > 0){
								$inner_array[] = "<span class='fa fa-plus ng-scope expand_tr' data-sku='".$sv['sku']."' data-accountcode='".$sv['fkaccountcode']."' data-marketplace='".$sv['fkaccountcode']."' ></span>";
								$i++;
							}else{
								$inner_array[] = '&nbsp;';
							}
							...
							foreach($cols_data as $k=>$v){ 
								if(isset($sv[$v['val']])){
									...
									elseif($v['val'] == 'saleprice'){
										$inner_array[] = '<label style="width:100px" ><label class="inventory-price-alignment"><span> <strong>'.$currency.'</strong> </span></label><label class="inventory-price-textbox-alignment"><input type="text" id="'.$v['val'].'_'.$sv['sku'].'_3" value="'.$sv[$v['val']].'" data-marketplace="9" data-sku="'.$sv['sku'].'" class="form-control amzMinPriceClass"></label></label>';
									}
									else{
				...
				foreach($sku_details['inventorys'] as $sk=>$sv){
					...
					$inner_array = array();
					$inner_array[] = "<input type='checkbox' style='display:block;margin:5px auto;' name='myCheckboxes[]' value='".$sv['sku']."_3' data-sku='".$sv['sku']."' data-accountcode='".$sv['fkaccountcode']."' data-marketplace='3' name='sku_checkbox[]' value='sku_".$sv['sku']."' >";
					if(isset($sv['childitemcount']) && intval($sv['childitemcount']) > 0){
						$inner_array[] = "<span class='fa fa-plus ng-scope expand_tr' data-sku='".$sv['sku']."' data-accountcode='".$sv['fkaccountcode']."' data-marketplace='3' ></span>";
						$i++;
					}else{
						$inner_array[] = '&nbsp;';
					}
