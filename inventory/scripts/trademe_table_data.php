					...
					foreach($sku_details as $k=>$v){
						if(is_array($v) && count($v)>0){
							foreach($v as $sk=>$sv){
								...
								$inner_array = array();
								$inner_array[] = "<input type='checkbox' style='display:block;margin:5px auto;' name='myCheckboxes[]' value='".$sv['sku']."_8' data-sku='".$sv['sku']."' data-accountcode='".$sv['accountcode']."' data-marketplace='8' name='sku_checkbox[]' value='sku_".$sv['sku']."' >";
								if(isset($sv['childitemcount']) && intval($sv['childitemcount']) > 0){
									$inner_array[] = "<span class='fa fa-plus ng-scope expand_tr' data-sku='".$sv['sku']."' data-accountcode='".$sv['accountcode']."' data-marketplace='8' ></span>";
									$i++;
								}else{
									$inner_array[] = '&nbsp;';
								}
								...
								foreach($cols_data as $k=>$v){ 
									if(isset($sv[$v['val']])){
										if($v['val'] == 'active'){
											...
											$inner_array[] = '<label style="display:flex"><input '.$disabled.' type="text" id="'.$sv['sku'].'_8" value="'.$sv['totalquantity'].'" class="form-control cmnTotQtyClass" data-marketplace="8" data-sku="'.$sv['sku'].'" ></label>';
										}
										elseif($v['val'] == 'trademebuynowprice'){
											if($sv[$v['val']] > 0){
												$inner_array[] = '<label><label class="inventory-price-alignment"><span> <strong>'.$currency.'</strong> </span></label><label class="inventory-price-textbox-alignment"><input type="text" id="'.$v['val'].'_'.$sv['sku'].'_8" value="'.$sv[$v['val']].'" data-marketplace="8" data-sku="'.$sv['sku'].'" class="form-control amzMinPriceClass"></label></label>';
											}else{
												$inner_array[] = '<label><label class="inventory-price-alignment"><span> <strong>'.$currency.'</strong> </span></label>&nbsp;0.00</label>';
											}
										}
										else{
										...
										if($v['val'] == 'totalquantity' || $v['val']== 'totalQty'){
											...
											$inner_array[] = '<label style="display:flex"><input '.$disabled.' type="text" id="'.$sv['sku'].'_8" value="'.$sv['totalquantity'].'" class="form-control cmnTotQtyClass" data-marketplace="8" data-sku="'.$sv['sku'].'" ><em data-sku="'.$sv['sku'].'" class="fa fa-lg fa-location-arrow" style="color:#087cc3;"></em></label>';
										}else{
											$inner_array[] = '&nbsp;';
										}
									}
