...
if(isset($_SESSION['usercode']) &&isset($_SESSION['dbcode']) &&isset($_SESSION['usertoken'])){
	...
	if(!empty($cols_data)){
		...
		$inventorydashboard_URL = COMPETE_API_URL."Shopifyinventoryapi/api/ShopifyInventory/GetShopifyInventory";
		$request = "<getshopifyinventoryrequest>
									<usertoken>".$usertoken."</usertoken>
									<dbcode>".$dbcode."</dbcode>
									<responsetype>json</responsetype>
									<searchtext>".$search_query."</searchtext>
									<recordsperpage>".$limit."</recordsperpage>
									<pagenumber>".$offset."</pagenumber>
								</getshopifyinventoryrequest>";
		//echo $request;exit;
		...
		if(!empty($sku_details)){
			$recordsTotal = $sku_details['totalnoofrecords'];
			if(!empty($search_query)){
				...
				foreach($sku_details as $k=>$v){
					if(is_array($v) && count($v)>0){
						foreach($v as $sk=>$sv){
							...
							$inner_array = array();
							$inner_array[] = "<input type='checkbox' style='display:block;margin:5px auto;' name='myCheckboxes[]' value='".$sv['sku']."_15' data-sku='".$sv['sku']."' data-accountcode='".$sv['accountCode']."' data-marketplace='15' name='sku_checkbox[]' value='sku_".$sv['sku']."' >";
							...
							foreach($cols_data as $k=>$v){ 
								if(isset($sv[$v['val']])){
									if($v['val'] == 'active'){
										...
										$inner_array[] = '<label style="display:flex"><input id="'.$sv['sku'].'_15" type="text" value="'.$sv[$v['val']].'" class="form-control cmnTotQtyClass" data-marketplace="13" data-sku="'.$sv['sku'].'" ></label>';
									}else{
				...
				$i=0;
				foreach($sku_details['shopifyinventories'] as $sk=>$sv){
					if(isset($sv['flagid']) && $sv['flagid'] != "" && $sv['flagid'] != ""){
						$flag_sku[$sv['sku']] = $sv['flagid'];
					}
					$inner_array = array();
