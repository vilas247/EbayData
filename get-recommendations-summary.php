...
// for type
	...
	$url = RECOMMENDATION_TYPE_API_URL."?type=".$_REQUEST['type'].'"&seller_ebay_id='.$seller_ebay_id;
	$sql = "select * from ebay_recommendations_data WHERE ebay_id='".$seller_ebay_id."' AND type='".$_REQUEST['type']."' ORDER BY id DESC LIMIT 1";
	...
	if(!empty($res) && mysqli_num_rows($res) > 0){
		...
		if(isset($check_errors->errors)){
			...
			if(json_last_error() === 0){
				$sql = "INSERT INTO ebay_recommendations_data(ebay_id,type,api_request,api_response) values('".$seller_ebay_id."', '".$_REQUEST['type']."','".$req."','".mysqli_real_escape_string($conn, $res)."')";
				mysqli_query($conn,$sql);
				print_r($res);exit;
			}else{
				echo "Error response<br/>";exit;
...
//for no value
	...
	$url = RECOMMENDATION_API_URL.'?seller_ebay_id='.$seller_ebay_id;
	$sql = "select * from ebay_recommendations_data WHERE ebay_id='".$seller_ebay_id."' AND type='all' ORDER BY id DESC LIMIT 1";
	...
	if(!empty($res) && mysqli_num_rows($res) > 0){
		...
		if(isset($check_errors->errors)){
			...
			if(json_last_error() === 0){
				$sql = "INSERT INTO ebay_recommendations_data(ebay_id,type,api_request,api_response) values('".$seller_ebay_id."', 'all','".$req."','".mysqli_real_escape_string($conn, $res)."')";
				mysqli_query($conn,$sql);
				print_r($res);exit;
			}else{
				echo "Error response<br/>";exit;
