...
if(count($_REQUEST)>0){
	if(isset($_REQUEST['campaign_id']) && isset($_REQUEST['campaign_name']) && isset($_REQUEST['start_date']) && isset($_REQUEST['end_date'])){
		...
		//API request
		$url = UPDATE_CAMPAIGN_API_URL."?seller_ebay_id=77_Champion_Dreams_Limited_eBay__eBay_UK";
		//$url = BASE_URL."campaign/update_campaign.php?seller_ebay_id=77_Champion_Dreams_Limited_eBay__eBay_UK";
		//echo $url;exit;
		$postData = array(
		...
		if ($conn->query($sql_ebay_campaign_api_request) === TRUE) {
			...
			$action_log = $campaign_name." is updated by user:".$user_id;
