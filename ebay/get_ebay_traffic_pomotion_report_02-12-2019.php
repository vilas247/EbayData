...
<!-- Content Starts -->  
	<div class="portlets-wrapper traditional whirl" style="margin-top:55px;min-height:1300px">
...
if(isset($_REQUEST['seller_ebay_id'])){
	...
	if(!empty($db_columns) ){
		...
		$response .= '<th class="hide" >Eligible Best Offer</th>';
		$response .= '</thead>';
		$response .= '<tbody>';
		
		$sql_val = "select * from ebay_traffic_pomotion_report WHERE ebay_seller_id='".$seller_ebay_id."' order by listing_impression_search_results_page DESC LIMIT 500";
		...
		if(!empty($res_val) && mysqli_num_rows($res_val) > 0){
			...
			while($values = $res_val->fetch_assoc()){
				...
				if(!empty($values['ebay_item_id'])){
					...
					$response .= "<td><input type='checkbox' class='checkboxes' style='display:block;margin:5px auto;' name='myCheckboxes[]' data-sku='".$values['item_sku']."' value='ebay_item_".$values['ebay_item_id']."' name='ebay_item_checkbox[]' ></td>";
					foreach($db_columns as $dbk=>$dbv){
						if(isset($values[$dbv['value']])){
							if($dbv['value'] == "item_name"){
								...
								if(is_null($values[$dbv['value']]) || $values[$dbv['value']] === NULL) {
									$response .= '<td><div id="ebay_value_'.$values['ebay_item_id'].'" >0%</div></td>';
								}
								else {
									$response .= '<td><div class="ebay_suggested_addrate" id="ebay_value_'.$values['ebay_item_id'].'" >'.$values[$dbv['value']].'%</div></td>';
								}
								
							}
							else if($dbv['value'] == "click_through_rate" || $dbv['value'] == "sales_conversion_rate"){
								$response .= '<td><div >'.$values[$dbv['value']].'%</div></td>';
							}
							else if($dbv['value'] == "item_bin_price"){
								$response .= '<td><div class="ebay_bin_price" id="ebay_item_bin_price_'.$values['ebay_item_id'].'" >'.$currency.$values[$dbv['value']].'</div></td>';
							}else{
								$response .= '<td>'.$values[$dbv['value']].'</td>';
							}
						}else{
							if($dbv['value'] == "select_ebay_value"){
								$response .= '<td><div class="display_flex">
								...
								$response .= '<td><select class="select-aspect" id="'.$values['ebay_item_id'].'_select_campaign" name="'.$values['ebay_item_id'].'_select_campaign"  class="form-control" ><option value"" >Select</option>'.$options.'</select></td>';
					...
					if(isset($values['is_eligible_offer'])){
						if($values['is_eligible_offer'] == 'Y'){
							$response .= '<td class="hide" >Eligible Best Offer</td>';
						}else{
							$response .= '<td class="hide">&nbsp;</td>';
						}
					}else{
						$response .= '<td class="hide">&nbsp;</td>';
					}
