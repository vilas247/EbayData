...
if(isset($_REQUEST['seller_ebay_id'])){
	...
	$sql_val = "select * from ebay_recommendation_enhancement_report WHERE ebay_seller_id='".$seller_ebay_id."' AND ((picture !='') OR (fnf !='') OR (expedited_shipping !='') OR (free_shipping !=''))";
					...
					if(isset($values[$dbv['value']])){
						if($dbv['value'] == "ebay_item_id"){
							...
							if(strlen($values[$dbv['value']]) > $name_size){
								$read_more = '<br><span data-toggle="collapse" data-target="#deno_'.$i.'" >Read More..</span><div class="collapse" id="deno_'.$i.'">'.$values[$dbv['value']].'</div>';
								$i++;
							}*/
							$response .= '<td style="width:450px;word-wrap: break-word"><p class="show-read-more" >'.$values[$dbv['value']].'</p></td>';
							//$response .= '<td style="width:450px;word-wrap: break-word"><p data-title="'.$values[$dbv['value']].'">'.replace_text_trim($name_size,$values[$dbv['value']]).$read_more.'</p></td>';
						}else if($dbv['value'] == "item_bin_price"){
							if(intval($values[$dbv['value']]) > 0){
								$response .= '<td>'.$currency.$values[$dbv['value']].'</td>';
							}else{
								$response .= '<td>Variation Parent</td>';
							}
