...
$sql = "select * from ebay_api_user_profile WHERE ebay_id='".$seller_ebay_id."' ORDER BY id DESC LIMIT 1";
...
if(!empty($res) && mysqli_num_rows($res) > 0){
	...
	$res = get_xml_response($header,array(),USER_PROFILE_API_URL.'?seller_ebay_id='.$seller_ebay_id);
	$req = USER_PROFILE_API_URL.'?seller_ebay_id='.$seller_ebay_id;
	$api_response = $res;
