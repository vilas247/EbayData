function ebayMilitaryInvoicePDF(data) {
	...
	for (var j = 0; j < data.length; j++) {
		...
		doc.addImage(X247Invoices.vistusmil, 'JPEG', 380, 150, 175, 125);
		...
		utvat = GrandTotalWithVatHaberCrafts(j, data[j].orderitems, data[j]);
		...
		var pp = 0.00;
		if (data[j].orderitems.length > 0) {
			pp = subtotal(j, data[j].orderitems);
		}
